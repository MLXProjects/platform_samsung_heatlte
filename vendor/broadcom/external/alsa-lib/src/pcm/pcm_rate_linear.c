...
static void rmc_shrink_s16(struct rate_linear *rate,
			      const snd_pcm_channel_area_t *dst_areas,
			      snd_pcm_uframes_t dst_offset, unsigned int dst_frames,
			      const snd_pcm_channel_area_t *src_areas,
			      snd_pcm_uframes_t src_offset, unsigned int src_frames)
{
	...
	for (channel = 0; channel < rate->channels; ++channel) {
		...
		input = malloc(sizeof(int16_t)*src_frames);
		if(input == NULL) {
			SNDERR("malloc error!!");
			break;
		}
		output = malloc(sizeof(int16_t)*dst_frames);
		if(output == NULL) {
			SNDERR("malloc error!!");
			free(input);
			break;
		...
		ret = rmc_resample(output, input, src_frames, RMC_SRC6_M);
...
static int linear_init(void *obj, snd_pcm_rate_info_t *info)
{
	...
	if (SND_PCM_STREAM_CAPTURE == info->stream)
		rmc_resampler_init(info->in.period_size);
	...
	if (info->in.rate < info->out.rate) {
		...
		if (info->in.format == info->out.format && info->in.format == SND_PCM_FORMAT_S16) {
			if ((info->stream == SND_PCM_STREAM_CAPTURE) &&
			    (info->in.rate == 48000) &&
			    (info->out.rate == 8000))
				rate->func = rmc_shrink_s16;
			else
...
{
	snd_pcm_stream_t stream = ((struct rate_linear *)(obj))->stream;
