...
/* Add PT */
/*!
  @brief	Control element list.
*/
static snd_ctl_elem_list_t g_snd_config_elist;
/*!
  @brief	Control handle.
*/
static snd_ctl_t *g_snd_ctl = NULL;
 ...
 * LSB 3.2
 */
static int __snd_config_update(int through_if_not_modified)
{
	int err;

#ifdef ENG_MODE
	struct stat current_conf_file_stat;
	static struct stat last_conf_file_stat;

	stat(SND_CONFIG_ASOUND_CTLFILE, &current_conf_file_stat);

	if (last_conf_file_stat.st_mtime != current_conf_file_stat.st_mtime) {
		SNDERR("asound.conf reloaded");
		last_conf_file_stat = current_conf_file_stat;
		snd_config_update_free_global();
	} else if (through_if_not_modified && snd_config) {
		SNDERR("snd_config_update_r() through");
		return 0;
	}
#else
	if (through_if_not_modified && snd_config) {
		SNDERR("snd_config_update_r() through");
		return 0;
	}
#endif
...
/*!
  ...
  @param	c [i] Sound config.
  @param	ctrl [i] Control.
  @param	client_name [i] Client name.

  @return	Function results.

  @note		.
*/
static int snd_config_check_value(snd_config_t *c,
					struct audio_test_codec_ctrl *ctrl,
					u_int client_name)
{
	const char *name = NULL, *id = NULL;
	int err, cnt, v;
	snd_config_iterator_t i, n;

	err = snd_config_get_id(c, &id);
	if (err < 0) {
		SNDERR("%s() ERROR!!! Invalid id", __func__);
		return err;
	}
	if (strcmp(id, "name") == 0) {
		err = snd_config_get_string(c, &name);
		if (err < 0) {
			SNDERR("%s() ERROR!!! Invalid type for %s",
							__func__, id);
			return err;
		}
		SNDMSG("%s() Found name '%s'", __func__, name);
		ctrl->name = name;
		return 0;
	}
	if (strcmp(id, "value") == 0) {
		if ((!strcmp(ctrl->name, "Path")) &&
		    (AUDIO_TEST_CLIENT_PT == client_name))
			return 0;
		/* Set codec */
		err = snd_config_set_codec(ctrl, c);
		if (err < 0) {
			SNDERR("%s() ERROR!!! snd_config_set_codec err[%d]",
				__func__, err);
			return err;
		}
		return 0;
	}
	SNDERR("%s() ERROR!!! Unknown field %s", __func__, id);
	return -EINVAL; /* Is EINVAL OK?? */
}
...
/*!
  ...
  @note		.
*/
int snd_config_check_values(const char *pcm_name, u_int client_name)
{
	char name[SND_CONFIG_MAX_NAME_LEN];
	snd_config_t *c_args;
	snd_config_iterator_t i, n, j, m;
	int err;
	struct audio_test_codec_ctrl ctrl;

	snprintf(name, SND_CONFIG_MAX_NAME_LEN,
			"pcm.%s.hooks.0.hook_args", pcm_name);

	err = __snd_config_update(1);
	if (err < 0) {
		SNDERR("%s() ERROR!!! cannot read alsaconf", __func__);
		return err;
	}

	err = snd_config_search_definition(snd_config, NULL, name, &c_args);
	if (err < 0) {
		SNDERR("%s() ERROR!!! '%s' is not found", __func__, name);
		return err;
	}

	snd_config_for_each(i, n, c_args) {
		memset(&ctrl, 0, sizeof(ctrl));
		snd_config_t *c_arg = snd_config_iterator_entry(i);
		snd_config_for_each(j, m, c_arg) {
			snd_config_t *c = snd_config_iterator_entry(j);
			err = snd_config_check_value(c, &ctrl, client_name);
			if (err < 0)
				return err;
		}
	}
	snd_config_delete(c_args);

	return 0;
}

/*!
  @brief	Set value to register of codec.

  @param	ctrl_tbl [i] Parameter of control.
  @param	c [i] Config data.

  @return	Function results.

  @note		.
*/
int snd_config_set_codec(struct audio_test_codec_ctrl *ctrl,
							snd_config_t *c)
{
	int ret = 0;
	...
	if (!ctrl) {
		SNDERR("%s() ERROR!!! Argument error", __func__);
		return -EINVAL;
	} else if (!ctrl->name) {
		SNDERR("%s() ERROR!!! Argument error", __func__);
		return -EINVAL;
	...
	/* Search elem_id */
	for (n = 0; n < g_snd_config_elist.count; n++) {
		if (strcmp((const char *)(g_snd_config_elist.pids[n].name),
						ctrl->name) == 0)
			break;
	}
	/* Read elem_value */
	ev.id = g_snd_config_elist.pids[n];
	ret = snd_ctl_elem_read(g_snd_ctl, &ev);
	if (ret < 0) {
		SNDERR("%s() ERROR!!! snd_ctl_elem_read ret[%d]", __func__, ret);
		goto error;
	}

	/* Read elem_info */
	einfo.id = g_snd_config_elist.pids[n];
	ret = snd_ctl_elem_info(g_snd_ctl, &einfo);
	if (ret < 0) {
		SNDERR("%s() ERROR!!! snd_ctl_elem_info ret[%d]", __func__, ret);
		goto error;
	}

	/* Create elem_value */
	if (c) {
		ret = snd_config_get_ctl_elem_value(c, g_snd_ctl, &ev,
							NULL, &einfo);
		if (ret < 0) {
			SNDERR("%s() ERROR!!! snd_config_get_ctl_elem_value(%d)",
			     __func__, ret);
			goto error;
		}
	} else {
		/* Create elem_value */
		for (n = 0; (n < einfo.count) &&
			(n < SND_CONFIG_MAX_ELEMENT); n++) {
			SNDERR("%s() value%d[%ld]",
				__func__, n, ctrl->elem_value[n]);
			switch (einfo.type) {
			case SND_CTL_ELEM_TYPE_BOOLEAN:
				ev.value.integer.value[n] =
						!!ctrl->elem_value[n];
				break;
			...
			case SND_CTL_ELEM_TYPE_ENUMERATED:
				ev.value.enumerated.item[n] =
						ctrl->elem_value[n];
				break;

			default:
				SNDERR("%s() ERROR!!! unknown type[%d]",
							__func__, einfo.type);
				ret = -EINVAL;
				goto error;
			}
...
//	dump_elem_values(&ev, &einfo);
//#endif
	ret = snd_ctl_elem_write(g_snd_ctl, &ev);
	if (ret < 0) {
		SNDERR("%s() ERROR!!! snd_ctl_elem_write ret[%d]",
							__func__, ret);
		goto error;
	}
...
error:
	SNDERR("%s() out ERROR!!! ret[%d]", __func__, ret);
	return ret;
}

/*!
  @brief	Create control and list of element ID.

  @param	p_eid_list [i] Pointer of list of element ID.

  @return	Function results.

  @note		.
*/
int snd_config_create_ctl_and_list(snd_ctl_elem_id_t **p_eid_list)
{
	int ret = 0;
	...
	memset(&g_snd_config_elist, 0, sizeof(g_snd_config_elist));

	ret = snd_ctl_open(&g_snd_ctl, ctl_name, 0);
	if (ret < 0) {
		SNDERR("Cannot open CTL %s", ctl_name);
		goto error;
	}

	ret = snd_ctl_elem_list(g_snd_ctl, &g_snd_config_elist);
	if (ret < 0) {
		SNDERR("%s() ERROR!!! snd_ctl_elem_list ret[%d]",
							__func__, ret);
		goto error;
	}

	/* Allocate eids */
	*p_eid_list = calloc(g_snd_config_elist.count,
				sizeof(snd_ctl_elem_id_t));
	if (!*p_eid_list) {
		SNDERR("%s() ERROR!!! snd_ctl_elem_id calloc", __func__);
		ret = -errno;
		goto error;
	}

	/* Set elem List */
	g_snd_config_elist.space = g_snd_config_elist.count;
	g_snd_config_elist.pids = *p_eid_list;
	ret = snd_ctl_elem_list(g_snd_ctl, &g_snd_config_elist);
	if (ret < 0) {
		SNDERR("%s() ERROR!!! snd_ctl_elem_list ret[%d]",
							__func__, ret);
...
/*!
  @brief	Delete control and list of element ID.

  @param	eid_list [i] List of element ID.

  @return	.

  @note		.
*/
void snd_config_delete_ctl_and_list(snd_ctl_elem_id_t *eid_list)
{
	/* Nothing to do */
}
