...
/* While PT loopback execution, we do not execute list_add_tail(). */
...
/* While PT loopback execution, we do not execute list_add_tail(). */
static int get_pt_state(void)
{
	int ret = AUDIO_TEST_DRV_STATE_OFF;
	int res = 0;
	int fd = -1;
	...
	res = ioctl(fd, AUDIO_TEST_IOCTL_GETLBSTATE, &cmd);
	if (0 != res) {
		SNDERR("ioctl is failed %d", errno);
		goto error;
	}
...
static int add_elem(snd_sctl_t *h, snd_config_t *_conf, snd_config_t *private_data)
{
	 ...
	 * Device-change is executed, so sound is output on device
	 * other than device specified in loopback command.
	 */
	if (AUDIO_TEST_DRV_STATE_ON == get_pt_state()) {
		err = -EINVAL;
		optional = 1;
		goto _err;
	}
