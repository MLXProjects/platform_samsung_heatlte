...
static inline void snd_pcm_strncpy(char *to, const char *from, size_t len)
{
	if (len > 0) {
		strncpy(to, from, len);
		*(to + len - 1)  = '\0';
	} else {
		SNDERR("Length error ! len[%d]", len);
	}
}

static int snd_pcm_set_pcmname(snd_pcm_t *pcm, u_int state)
{
	int res = 0;
	int fd = -1;
	...
	if (AUDIO_TEST_DRV_PCMSTATE_OPEN == state)
		snd_pcm_strncpy(pcmcmd.pcmname,
					pcm->name,
					AUDIO_TEST_PCMNAME_MAX_LEN);
	if (SND_PCM_STREAM_PLAYBACK == pcm->stream)
		pcmcmd.pcmdirection = AUDIO_TEST_DRV_PCMDIR_PLAYBACK;
	else
		pcmcmd.pcmdirection = AUDIO_TEST_DRV_PCMDIR_CAPTURE;

	res = ioctl(fd, AUDIO_TEST_IOCTL_SET_PCMNAME, &pcmcmd);
	if (0 != res)
		SNDERR("ioctl is failed %d", res);
...
{
	int res = 0, err;
	(void)snd_pcm_set_pcmname(pcm, AUDIO_TEST_DRV_PCMSTATE_CLOSE);
	assert(pcm);
...
/* #ifndef DOC_HIDDEN */
/* link_warning(snd_pcm_hwsync, "Warning: snd_pcm_hwsync()
	is deprecated, consider to use snd_pcm_avail()"); */
/* #endif */
		 ...
		 snd_pcm_stream_t stream, int mode)
{
	...
	if (err < 0) {
		SNDERR("snd_pcm_open_noupdate is failed[%d]", err);
		return err;
	}
	err = snd_pcm_set_pcmname(*pcmp, AUDIO_TEST_DRV_PCMSTATE_OPEN);
	if (0 != err)
		SNDERR("snd_pcm_set_pcmname is failed[%d]", err);
	return err;
}
